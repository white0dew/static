...
适用于通过 FTP / 虚拟主机部署的纯静态站点，尤其是 `HTML + CSS + JS + 图片` 资源。

### 💡 核心规则

- ** 静态资源防缓存 **：每次发布后，若 `CSS`、`JS`、图片内容发生变化，必须同步更新资源引用中的版本号查询参数，例如 `?v=20260404-1`
- ** 查询参数不改变实际文件路径 **：`css/style.css?v=20260404-1` 实际读取的仍然是 `css/style.css`
- ** 浏览器缓存键按完整 URL 区分 **：当 `v` 变化时，浏览器会将其视为新资源并重新请求
- ** HTML 入口优先更新 **：若资源已上传但页面仍显示旧样式 / 旧脚本，优先检查 `index.html` 中的资源版本号是否已更新

### 🔧 推荐写法

```html
<link rel="stylesheet" href="css/style.css?v=20260404-1" />
<script src="js/main.js?v=20260404-1" defer></script>
<img class="avatar-photo" src="public/icon.jpg?v=20260404-1" alt="头像" />
```

### ⚠️ 发布注意事项

- ** 改内容就改版本号 **：只更新文件内容但不更新 `v`，浏览器可能继续使用旧缓存
- ** 保持版本号可追踪 **：推荐格式为 `YYYYMMDD-N`，例如 `20260404-1`
- ** 若接入 CDN / 面板缓存 **：除更新 `v` 外，还应视情况执行 `Purge Cache` 或 `Clear Cache`
- ** FTP 上传完成后要回读校验 **：至少确认远端 `HTML` 与关键静态资源已更新到预期版本
